...
Public Class FormPenjualan
    ...
    Sub TampilPenjualan()
        Call Koneksi()
        da = New SqlDataAdapter("select kode_jual, nama_mobil, nama_pembeli, tb_penjualan.harga, jumlah, total_dibayar, tb_penjualan.tanggal_beli from tb_penjualan inner join tb_mobil on tb_penjualan.kode_mobil = tb_mobil.kode_mobil", konek)
        ds = New DataSet
        ds.Clear()
    ...
    End Sub
    Sub TampilMobil()
        cmd = New SqlCommand("SELECT kode_mobil, nama_mobil, harga FROM tb_mobil", konek)
    ...
    Private Sub bTambah_Click(sender As Object, e As EventArgs) Handles bTambah.Click
        ...
        If tNamaPbl.Enabled = False Then
            ...
            cbKodeMbl.Focus()
        ElseIf tKodeJ.Text = "" Or tNamaPbl.Text = "" Or cbKodeMbl.Text = "" Or tHarga.Text = "" Or tJumlah.Text = "" Or tTotal.Text = "" Then
            MsgBox("Data Belum Lengkap !")
        Else
            Call Koneksi()
            Dim Simpan As String
            Simpan = "INSERT INTO tb_penjualan VALUES('" & tKodeJ.Text & "','" & cbKodeMbl.Text.Substring(0, 5) & "','" & tNamaPbl.Text & "','" & tHarga.Text & "','" & tJumlah.Text & "','" & tTotal.Text & "','" & Date.Today() & "')"
    ...
    Private Sub bUbah_Click(sender As Object, e As EventArgs) Handles bUbah.Click
        ...
        ElseIf tNamaPbl.Enabled = True And bTambah.Enabled = False Then
            Call Koneksi()
            Dim Edit As String
            Edit = "UPDATE tb_penjualan SET kode_mobil='" & cbKodeMbl.Text.Substring(0, 5) & "', nama_pembeli='" & tNamaPbl.Text & "', harga='" & tHarga.Text & "', jumlah='" & tJumlah.Text & "', total_dibayar='" & tTotal.Text & "' WHERE kode_jual='" & tKodeJ.Text & "'"
    ...
    Private Sub DataGridView1_DoubleClick(sender As Object, e As EventArgs) Handles DataGridView1.DoubleClick
        ...
        cmd = New SqlCommand("select kode_jual, tb_penjualan.kode_mobil, nama_mobil, nama_pembeli, tb_penjualan.harga, jumlah, total_dibayar, tb_penjualan.tanggal_beli from tb_penjualan inner join tb_mobil on tb_penjualan.kode_mobil = tb_mobil.kode_mobil WHERE kode_jual='" & DataGridView1.Item(0, i).Value & "'", konek)
        rd = cmd.ExecuteReader
        rd.Read()
        If Not rd.HasRows Then
            tNamaPbl.Focus()
    ...
    Private Sub tSearch_TextChanged(sender As Object, e As EventArgs) Handles tSearch.TextChanged
        Call Koneksi()
        cmd = New SqlCommand("SELECT kode_jual, nama_mobil, nama_pembeli, tb_penjualan.harga, jumlah, total_dibayar, tb_penjualan.tanggal_beli FROM tb_penjualan INNER JOIN tb_mobil on tb_penjualan.kode_mobil = tb_mobil.kode_mobil WHERE nama_mobil LIKE '%" & tSearch.Text & "%' OR kode_jual LIKE '%" & tSearch.Text & "%' OR nama_pembeli LIKE '%" & tSearch.Text & "%'", konek)
        rd = cmd.ExecuteReader
        rd.Read()
        If rd.HasRows Then
            Call Koneksi()
            da = New SqlDataAdapter("SELECT kode_jual, nama_mobil, nama_pembeli, tb_penjualan.harga, jumlah, total_dibayar, tb_penjualan.tanggal_beli FROM tb_penjualan INNER JOIN tb_mobil on tb_penjualan.kode_mobil = tb_mobil.kode_mobil WHERE nama_mobil LIKE '%" & tSearch.Text & "%' OR kode_jual LIKE '%" & tSearch.Text & "%' OR nama_pembeli LIKE '%" & tSearch.Text & "%'", konek)
            ds = New DataSet
            da.Fill(ds, "ketemu")
    ...
    Private Sub cbKodeBrgJ_TextChanged(sender As Object, e As EventArgs) Handles cbKodeMbl.TextChanged
        If cbKodeMbl.Text.Length >= 5 And bUbah.Enabled = False And bHapus.Enabled = False Then
            Dim kode_barang As String
            kode_barang = cbKodeMbl.Text.Substring(0, 5)
            Call Koneksi()
            cmd = New SqlCommand("SELECT nama_mobil, harga FROM tb_mobil WHERE kode_mobil ='" & kode_barang & "'", konek)
            rd = cmd.ExecuteReader
            rd.Read()
            tHarga.Text = rd.Item("harga")
        ElseIf cbKodeMbl.Text.Length >= 5 And tNamaPbl.Enabled = True And bTambah.Enabled = False And bHapus.Enabled = False Then
            Dim kode_barang As String
            kode_barang = cbKodeMbl.Text.Substring(0, 5)
            Call Koneksi()
            cmd = New SqlCommand("SELECT nama_mobil, harga FROM tb_mobil WHERE kode_mobil ='" & kode_barang & "'", konek)
    ...
    Private Sub cbKodeBrgJ_KeyPress(sender As Object, e As KeyPressEventArgs) Handles cbKodeMbl.KeyPress
        e.KeyChar = Chr(0)
    End Sub
End Class